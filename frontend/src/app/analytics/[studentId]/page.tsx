  ...
  return (
    <div>
      ...
      <div className="flex items-center gap-2 text-sm text-slate-400 mb-6">
        ...
        <span>/</span>
        <Link href={`/students/${studentId}`} className="hover:text-slate-600">{analytics.student_name}</Link>
        <span>/</span>
      ...
      {/* Header */}
      <div className="mb-8">
        <h1 className="text-2xl font-bold text-slate-900">Analytics - {analytics.student_name}</h1>
        <p className="text-slate-500 mt-1">Detailed progress and performance overview</p>
      </div>
      ...
      <div className="bg-white rounded-xl border border-slate-200 p-6 mb-8">
        <h2 className="text-lg font-semibold text-slate-900 mb-5">Deficit Area Progress</h2>
        <div className="space-y-6">
          {analytics.deficit_progress.map((dp) => {
            ...
            return (
              <div key={dp.area} className="border border-slate-100 rounded-xl p-4">
                ...
                <div className="grid grid-cols-2 gap-4">
                  ...
                  />
                  <div>
                    <p className="text-sm font-medium text-slate-700 mb-1.5">Accuracy Trend</p>
            ...
            <table className="w-full">
              <thead>
                <tr className="border-b border-slate-100">
                  <th className="text-left text-xs font-medium text-slate-400 uppercase tracking-wider py-3 px-2">Game</th>
                  <th className="text-left text-xs font-medium text-slate-400 uppercase tracking-wider py-3 px-2">Area</th>
                  <th className="text-left text-xs font-medium text-slate-400 uppercase tracking-wider py-3 px-2">Accuracy</th>
                  <th className="text-left text-xs font-medium text-slate-400 uppercase tracking-wider py-3 px-2">Points</th>
                  <th className="text-left text-xs font-medium text-slate-400 uppercase tracking-wider py-3 px-2">Status</th>
                  <th className="text-left text-xs font-medium text-slate-400 uppercase tracking-wider py-3 px-2">Date</th>
                </tr>
              </thead>
              <tbody>
                {analytics.recent_sessions.map((s) => {
                  ...
                  return (
                    <tr key={s.id} className="border-b border-slate-50 hover:bg-slate-50">
                      <td className="py-3 px-2 text-sm font-medium text-slate-900">{s.game_name}</td>
