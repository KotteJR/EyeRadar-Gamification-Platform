...
import DungeonOverlay from "@/components/phaser/DungeonOverlay";
...
const FALLBACK_ITEMS: ExerciseItem[] = [
  {
    index: 0,
    item_type: "castle_dungeon",
    question: "Which word rhymes with 'cat'?",
    options: ["hat", "dog", "sun", "map"],
    correct_answer: "hat",
  ...
  if (loading) {
    return (
      <div className="fixed inset-0 bg-black flex items-center justify-center">
        <div className="text-center">
          <div className="w-12 h-12 border-3 border-amber-500 border-t-transparent rounded-full animate-spin mx-auto mb-4" />
          <p className="text-white/60 text-sm font-semibold">Preparing castle challenge...</p>
        </div>
      </div>
    );
  }

  const items = session ? session.items.slice(0, 5) : FALLBACK_ITEMS;
  const sessionId = session?.id || "test-session";

  const levelConfig = {
    worldTheme: "cloud_kingdom",
    bossType: "castle_dungeon",
...
export default function CastleBossPage() {
  return (
    <Suspense
      fallback={
        <div className="fixed inset-0 bg-black flex items-center justify-center">
